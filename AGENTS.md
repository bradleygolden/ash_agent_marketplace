...
- Keep unit tests in `test/ash_agent_marketplace`, mirroring `lib/` structure with `<filename>_test.exs`, and default to `async: true` when isolation is possible.
...
```
test/
├── ash_agent_marketplace/           # Unit tests (mirror lib/ structure)
...
defmodule AshAgentMarketplace.Integration.Metadata.ReqLLM.OpenAITest do
  use AshAgentMarketplace.IntegrationCase, backend: :req_llm, provider: :openai
